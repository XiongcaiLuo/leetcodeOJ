...
public class SymbolDigraph {
    ...
    public SymbolDigraph(List<Pair<String>> edges) {
        st = new ST<String, Integer>();

        for (Pair<String> p : edges){
        	if (!st.contains(p.getFirst()))
        		st.put(p.getFirst(), st.size());
        	if (!st.contains(p.getSecond()))
        		st.put(p.getSecond(), st.size());
        }
        ...
        for (Pair<String> p : edges){
        	int v = st.get(p.getFirst());
        	int w = st.get(p.getSecond());
        	G.addEdge(v, w);
        }
